...
public class DirectedLocationOverlay extends Overlay
{
  ...
  public void draw(Canvas paramCanvas, MapView paramMapView, boolean paramBoolean)
  {
    ...
    localProjection.toMapPixels(this.mLocation, this.screenCoords);
    if ((this.mShowAccuracy) && (this.mAccuracy > 10))
    {
      float f = localProjection.metersToEquatorPixels(this.mAccuracy);
      if (f > 8.0F)
      {
    ...
    Bitmap localBitmap = Bitmap.createBitmap(this.DIRECTION_ARROW, 0, 0, this.DIRECTION_ARROW_WIDTH, this.DIRECTION_ARROW_HEIGHT, this.directionRotater, false);
    paramCanvas.drawBitmap(localBitmap, this.screenCoords.x - localBitmap.getWidth() / 2, this.screenCoords.y - localBitmap.getHeight() / 2, this.mPaint);
  }
 ...
 * Qualified Name:     org.osmdroid.views.overlay.DirectedLocationOverlay
